...
sub set_address {
    ...
    my $address = sprintf("opensrf:client:%s:%s:", $self->{domain}, hostfqdn());

    # Add the service name for service clients since it can be a 
    # useful debugging tool.
    $address .= "$service:" if $service;

    $address .= "$$:";
    $address .= int(rand(10_000));
